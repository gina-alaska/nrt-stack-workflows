%w{aqua terra}.each do |satellite|
  ...
  # Scripts expect aqua-1 or terra-1 for platform, not aqua, terra :(
  steps[:tdf] = Step.where(name: "#{satellite.capitalize}TDF").first_or_create({
    command: "modis_tdf.rb -p #{satellite}-1 -t {{workspace}} {{job.input_path}} {{job.output_path}}",
    queue: 'terascan',
    processing_level: ProcessingLevel.where(name: 'tdf').first_or_create,
    sensor: Sensor.where(name: 'modis').first_or_create,
    parent: steps[:seadas]
    })
  steps[:tdf].requirements = %w{terrascan}.map do |requirement|
    Requirement.where(name: requirement).first_or_create
  end
  ...
  steps[:awips] = Step.where(name: "#{satellite.capitalize}Awips").first_or_create({
    command: "terra_and_aqua_awips.rb -p 8 -t {{workspace}} {{job.input_path}} {{job.output_path}}",
    queue: 'polar2grid',
    sensor: Sensor.where(name: 'modis').first_or_create,
  ...
  steps[:ldm] = Step.where(name: "#{satellite.capitalize}LDMInsert").first_or_create({
  ...
  steps[:awips].requirements = %w{ldm}.map do |requirement|
    Requirement.where(name: requirement).first_or_create
  end
